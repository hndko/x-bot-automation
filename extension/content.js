...
async function undoRetweets(settings) {
  try {
    ...
    while (stats.processed < settings.maxActions && emptyScrollCount < 5) {
        ...
        try {
          ...
          if (unretweetButton) {
            ...
            if (settings.dryRun) {
              ...
              stats.processed++;
              sendProgress(
                `🧪 DRY RUN: Would undo ${stats.success}/${stats.processed} retweets`,
                stats.processed,
                settings.maxActions,
                stats.success,
                stats.failed
      ...
      // If no retweeted tweets found in this batch, scroll for more
      if (!foundRetweetedTweet) {
        consecutiveSkips++;
        console.log(
          `⚠️ No retweeted tweets in current view (${consecutiveSkips}/3)`
        );

        if (consecutiveSkips >= 3) {
          console.log("❌ No more retweeted tweets found after 3 scrolls");
          break;
        }
    ...
    if (stats.processed >= settings.maxActions) {
      ...
      stopReason = "(no more retweeted tweets)";
    }

    await clearProgress();

    sendProgress(
      `🎉 Complete! Undone ${stats.success} of ${stats.processed} retweets ${stopReason}`,
      stats.processed,
      stats.processed,
      stats.success,
      stats.failed
