...
const sounds = {
  success: new Audio(
    "data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBjGJ0fPTgjMGHm7A7+OZUBAM"
  ),
  error: new Audio(
    "data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBjGJ0fPTgjMGHm7A7+OZUBAM"
  ),
...
// NEW: Delete Replies handler
document
  .getElementById("btnDeleteReplies")
  ?.addEventListener("click", async () => {
    if (isRunning) return;

    const confirmMsg = dryRunCheckbox.checked
      ? "🧪 DRY RUN: Preview which replies would be deleted?"
      : "⚠️ Delete your replies? This cannot be undone!";

    if (confirm(confirmMsg)) {
      await sendMessageToTab("delete_replies");
    }
  });

// NEW: Unfollow Non-Followers handler
document
  .getElementById("btnUnfollowNonFollowers")
  ?.addEventListener("click", async () => {
    if (isRunning) return;

    const confirmMsg = dryRunCheckbox.checked
      ? "🧪 DRY RUN: Preview who doesn't follow back?"
      : "👥 Unfollow users who don't follow you back?";

    if (confirm(confirmMsg)) {
      await sendMessageToTab("unfollow_non_followers");
    }
  });
